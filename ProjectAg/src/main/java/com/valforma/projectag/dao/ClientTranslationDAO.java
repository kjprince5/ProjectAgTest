...
public class ClientTranslationDAO extends AbstractDAO<ClientTranslation, ClientTranslation> {
	...
	public Long getCountForResolutionViaCode(String code, BigInteger clientId) {
		BigInteger value = (BigInteger) entityManager
				.createNativeQuery("select count(*) from CLIENT_TRANSLATION where code like '" + code
						+ "' and RECIPROCAL = 1 and client_id = " + clientId + ";")
				.getSingleResult();
		return value.longValue();

	}

	public Long getCountForResolutionViaCodeExceptThis(String code, BigInteger id, BigInteger clientId) {
		BigInteger value = (BigInteger) entityManager
				.createNativeQuery("select count(*) from CLIENT_TRANSLATION where code like '" + code
						+ "' and RECIPROCAL = 1 and id not in (" + id + ") and client_id = " + clientId + ";")
				.getSingleResult();
		return value.longValue();
